...
import (
	...
	"github.com/titus12/ma-commons-go/gcache/bigcache/queue"
)
...
func (s *cacheShard) getAndRefreshExpire(key string, hashedKey uint64) ([]byte, error) {
	currentTime := uint64(s.clock.epoch())
	s.lock.RLock()
	wrappedEntry, err := s.getWrappedEntry(hashedKey)
	if err != nil {
	...
	s.lock.RUnlock()
	s.lock.Lock()
	writeTimestampToEntry(wrappedEntry, currentTime)
	s.lock.Unlock()
	s.hit(hashedKey)
	return entry, nil
}
